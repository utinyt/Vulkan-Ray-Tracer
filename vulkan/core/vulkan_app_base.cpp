...
void VulkanAppBase::update() {
	...
	static int oldGraveAccentState = GLFW_RELEASE;
	if (glfwGetKey(window, GLFW_KEY_GRAVE_ACCENT) == GLFW_PRESS && oldGraveAccentState == GLFW_RELEASE) {
		captureMouse = !captureMouse;
		glfwSetInputMode(window, GLFW_CURSOR, captureMouse ? GLFW_CURSOR_DISABLED : GLFW_CURSOR_NORMAL);
		oldGraveAccentState = GLFW_PRESS;
	}
	if (glfwGetKey(window, GLFW_KEY_GRAVE_ACCENT) == GLFW_RELEASE && oldGraveAccentState == GLFW_PRESS) {
		oldGraveAccentState = GLFW_RELEASE;
	}
